...
parser.add_argument("--reference_frame", default="0001", type=str)
...
# Create input directory on a certain timestep
print("Creating input directory with reference frame...")

frames_path = os.path.join(args.source_path)
cameras = os.listdir(frames_path)

os.makedirs(os.path.join(args.source_path, "input"), exist_ok=True)

ref_frame_path = os.path.join(frames_path, cameras[0], "images", args.reference_frame + ".png")
print(ref_frame_path)
ref_frame = cv2.imread(ref_frame_path)

for camera in cameras:

    if camera == "input" or camera == "distorted":
        continue

    selected_frame = os.path.join(frames_path, camera,"images", args.reference_frame + ".png")
    input_path = os.path.join(args.source_path, "input")

    frame = cv2.imread(selected_frame)
    frame = cv2.resize(frame, (ref_frame.shape[1], ref_frame.shape[0]), interpolation = cv2.INTER_AREA)

    output_path = os.path.join(input_path, camera + ".png")

    cv2.imwrite(output_path, frame)

    print("Wrote " + selected_frame + " to " + input_path)
...
mapx, mapy, roi, undistorted_ref = get_undistortion(args.source_path)
output_path = os.path.join(args.source_path, args.output)
...
poses, pts3d, perm = load_colmap_data(args.source_path)
